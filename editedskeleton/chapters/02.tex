...
\author{Effie Mouka, Ioannis E. Saridakis, Angeliki Fotopoulou}
...
\begin{itemize}
\item Do the Right Thing \citep{Lee1989};
\item American History X \citep{Kaye1998};
\item Monster's Ball \citep{Forster2001};
\item Crash \citep{Haggis2004};
\item This is England \citep{Meadows2006}.
\end{itemize}
...
\item utterances: information on the speaker (e.g., SPK1 [Danny]), the time-stamp (\textit{anchorsynch}), and the dependent/aligned tiers with the Greek and Spanish translations respectively (\textit{spanGrp} type).
\end{itemize}

An example of an XML-tagged utterance and of its translations into EL and ES, as outputted from ELAN following processing in TEI-Drop is as follows:
...
\subsection{Attitude types and usage perspective}
...
\noindent (1) Your mother, she \underline{hated them niggers} too (\textit{Monster's Ball})\newline
\noindent (2) That means \underline{not welcome} (\textit{American History X}) [utterance addressed to a Jew while the speaker reveals his swastika tattoo]\newline
\noindent (3) Take your fucking pizza piece and \underline{go the fuck back to Africa} (\textit{Do the Right Thing})\newline
\noindent (4) Yeah, \underline{fuck off, you Paki bastards} (\textit{This is England})\newline
\noindent (5) \underline{What the hell are those niggers} doing out there? (\textit{Monster's Ball})

\subsubsection{Appreciation}

\noindent (6) We'll let the \underline{niggers, kikes and spics} grab for their piece of the pie (\textit{American History X})\newline
\noindent (7) a \underline{bunch of people} who \textit{aren't even citizens of this country} [...] (\textit{American History X})\newline
\noindent (8) She \underline{smells like fish and chips and guacamole} (\textit{American History X})\newline
\noindent (9) Three and a half million of us, who can't find fucking work because \underline{they're taking them all, because it's fucking cheap labour} (\textit{American History X})\newline
\noindent (10) How come \underline{niggers are so stupid}? (\textit{Do the Right Thing})\newline
\noindent (11) Magic, Eddie, Prince, are not \underline{niggers}. I mean, they're not \textit{black}, I mean... (\textit{Do the Right Thing})

\subsubsection{Judgement}


\noindent (12) Look at these little \underline{fucking sewer rats} (\textit{This is England}) [referring to young immigrants playing in a yard]
\newline
\noindent (13) \underline{Immigration, AIDS, welfare, those are problems of the black community, the Hispanic community, the Asian community} (\textit{American History X})
\newline
\noindent (14) One in every three black males is in some phase of the correctional system. Is that a coincidence or do these people \underline{have like a racial commitment to crime}? (\textit{American History X})
\newline
\noindent (15) All right. Well, you know what I can't do? I can't look at you without thinking about the \underline{five or six more qualified white men who didn't get your job} (\textit{Crash})
\newline
\noindent (16) He's one of those \underline{proud to be nigger people} (\textit{American History X})\newline


Although “interpersonal epithets” \citep[see][376-377]{Halliday2014}, e.g. evaluative adjectives, are the most obvious evaluative device of language, the lexico-grammatical choices that express attitude are infinite, especially if we consider that evaluative uses of language can be present in discourse both explicitly and implicitly \citep[cf.][23] {Munday2012}. As shown in the above examples, the lexico-grammatical means to express attitude are vast and not limited to closed semantic or grammatical categories.
...
\begin{itemize}
\item used to express mere anger and hate, as in (1) and (5),
\item used in appreciations just to refer to Others in a disparaging manner, presenting the fact of belonging to other racial groups and/or having their “inherent” characteristics as being \textit{per se} negative, as in (6) and (11), or
\item used in judgements to negatively evaluate a certain person's behaviour, which is presented as related to the fact that he/she is black (12).
\end{itemize}

The interpretation of each instance is based on the context of situation, and is further based on the presence of various markers, either explicitly stated, as in (1) where the verb \textit{hate} is used, or using interjections such as \textit{go the fuckback}, \textit{fuck off} and \textit{what the hell} [in (3), (4), and (5)].

On the other hand, in (6) racial slurs are used instead of “neutral” ethnic denominations as disparaging terms, simply to mark the inferiority of the mentioned groups. The utterance in example (11) is a response to the interlocutor's argument that, despite his constant negative attitude towards black people, all his favourite celebrities (Eddie Murphy, Magic Johnson and Prince) are blacks. It is a representative example of how a highly marked racial slur is used as a negative appreciation, to evaluate people as being “nice” or “bad”.

Accordingly, our data includes many convictions and stereotyped visions of Others (7) and recurrent topics or \textit{topoi}, as “common-sense reasoning [that is] typical for specific issues” \citep{Dijk2000c} qt. in \citet[299, note 21]{BakerGabrielatos2008}; \citep[see also][74–76]{Reisigl2001}. Examples are the \textit{topos} of finance (9), the \textit{topos} of threat (13, 14) and the \textit{topos} of justice or equal opportunities (15). Such visions can be used as appreciations or judgements, i.e. presented as either negative phenomena, as in (9), or as criticism of the behaviour of Others, as in (13), (14) and (15).
...
\noindent (17) You \textit{gold-teeth, gold-chain-wearing, fried-chicken and biscuit-eating monkey, ape, baboon, big-thigh, fast running, high-jumping, spear chucking, 360-degree basketball-dunking, titsoon, spade, moulignon}!
\newline
...
\subsection{\textit{Attitude features}}

Attitudes are also analysed in terms of its features.
...
Thus, in example (8) above, one should know that \textit{fish and chips and guacamole} refer to the culinary traditions of Latin Americans in order to properly interpret it, whereas in example (12) knowledge about the policies of affirmative action against racial discrimination in the USA is crucial in order to interpret the utterance correctly. Moreover, in example (12) the term \textit{sewer rat} is used metaphorically to designate useless people that cause problems to society.

\subsubsection{Irony}

Irony is also a case of implicit meaning, a pragmatic phenomenon used to express a meaning contrary or different to the literal one. Once again, its recognition depends on the situational context which indicates something different than the apparent \citep[123]{Fairclough1992}. For instance, the utterance in example 18 is used to depreciate African-American literature:
\newline
\noindent (18) What is it, Black History Month? (\textit{American History X})
...
\noindent (19) Man's singing about lynching niggers. “Gonna buy me a rope and lynch me a nigger” (\textit{Crash})\newline
\noindent (20) Your partner's a racist prick (\textit{Crash})
...
\subsection{Annotation Scheme Overview}
The attitude classification outlined above and used as annotation scheme in the reported project can be schematised as follows:

\begin{figure}
\includegraphics[width=.5\textwidth]{./figures/4-3.png} 
\caption{Corpus annotation scheme}
\end{figure}

\section{Register shifts: Analysis and examples}
...
\noindent (21)\newline
\noindent \textbf{[EN]} And now some fucking Korean owns it who fired \underline{these guys} and is making a killing because he hired \underline{40 fucking border jumpers}\newline
\noindent \textbf{[EL]} Τώρα το 'χει ένας Κορεάτης, που απέλυσε τους \underline{δικούς μας} και θησαυρίζει επειδή προσέλαβε \underline{λαθρομετανάστες}\newline 
\noindent \textbf{[Back translation]} Now a Korean owns it who fired \underline{our guys }and is making a killing because he hired \underline{illegal immigrants} \newline
...
In example (23) above, we mark the use of three racial slurs, \textit{nigger} (enTenTen12 frequency: 0.9 per million) as highly negative appreciations towards Jews, Latinos, and black people respectively. In the Spanish TT, the racial slurs of the original are shifted towards neutralisation (\textit{negros and latinos}), while the reference to Jews (\textit{kikes}) is eliminated. In all, the negative appreciations that are inherent in racial slurs are eliminated. Although in Spanish there is a lexeme, \textit{negrata}, which has the same derogatory connotation as \textit{nigger}, translating \textit{nigger} as \textit{negro} is a recurrent practice in our corpus. However, \textit{negrata} is a term with a very low frequency (only 97 tokens in esTenTen11), while \textit{nigger} appears with a frequency of 0.9 per million in enTenTen12. This observation could be indicative of the reasons that made Spanish subtitlers opt for the neutral term, avoiding the use of an uncommon term. On the other hand, \textit{spic}, a term that could have been translated as \textit{sudaca} (a Spanish racial slur with presumably similar connotations with 431 tokens in esTenTen11) is also avoided. In both cases, the translator opts for neutralising the rendition of the original utterance.\newline

\noindent (24)
\newline
\noindent \textbf{[EN]} Name your price, \textit{cracker}.\newline
\noindent
\textbf{[ES]} Di tu precio, \textit{blanco}.\newline
\noindent
\textbf{[Back translation]} Name your price, white guy.\newline
\noindent
\textbf{[EL]} Πες το ποσόν, \textit{βλάχο}.\newline
\noindent
\textbf{[Back translation]} Name your price, country bumpkin.\newline


During a basketball game in the neighbourhood court, members of the skinhead gang start to quarrel with members of a “black gang”. Derek has a bet; he proposes a “whites against blacks” game. The answer in (24) comes from one of the opponents, indicating that they accept the bet.
In example (24) above, the disparaging term \textit{cracker} (showing negative appreciation, i.e. for a poor white person, usually from the South) is translated as \textit{blanco} [white] in Spanish, but as \textit{βλάχο}[country bumpkin] in Greek. In this case, the Spanish subtitler succeeds in maintaining the racial nuance of the term, although the strength of the negative appreciation is diminished, while the Greek subtitler eliminates the racial reference and only transfers the aggressive tone of the dialogue. \newline

\noindent (25)
\newline
\noindent \textbf{[EN]} -Do you speak English? -I am speaking English, you stupid cow!\newline
\noindent \textbf{[EL]}-Θα μιλήσετε Αγγλικά; -Μιλάω Αγγλικά!\newline
\noindent \textbf{[Back translation]} -Are you going to speak English? -I speak English!\newline
...
\noindent (26) \newline
\noindent \textbf{[EN]} Stupid \textit{wetback}\newline
\noindent \textbf{[ES]} Estúpida \textit{sin papeles}\newline
\noindent \textit{[Back translation]} Stupid undocumented immigrant\newline

In (26), an Asian and a Latin American woman are involved in a car crash. While they quarrel, the first one calls the other a wetback. The term is highly disparaging and refers to illegal Latin Americans (especially Mexicans) as a descriptor of the way Mexicans enter the US, crossing the Rio Grande. Therefore, it has been marked as a negative judgement utterance. Although the term is decades-old and has been used even in the title of a deportation programme of the US in 1954 (the so-called “Operation Wetback”); \citep[see][]{Hernandez2006}, today it is used in a highly derogatory manner as a racial slur. As shown in Fig. 5, the most significant collocations of wetback (sorted by Mutual-Information, in a query window of {-10 to +10} tokens) are other racial slurs, especially in their context of usage (e.g. \textit{mojado}, \textit{beaners}, \textit{spics}, \textit{kike}, \textit{nigger}, \textit{Chink}, \textit{greaser}, \textit{lowlife}, etc.).

\begin{figure}
\includegraphics[width=.5\textwidth]{./figures/4-5.png}
\caption{Collocates of wetback in enTenTen12, sorted by Mutual Information (MI) in SketchEngine}
\end{figure}

On the other hand, the Spanish translation uses sin papeles [undocumented immigrants]. This is a rather neutral term to refer to illegal immigrants. In esTenTen11, the most significant collocates of sin papeles (see Fig. 6) are emotionally neutral (\textit{inmigrante} [immigrants], \textit{empadronar}/\textit{empadronamiento} [inclusion in the town registry], \textit{redadas} [raids], \textit{patera}/\textit{pateras} [dinghy]) and an analysis of the concordances shows that the term is used also in texts, in support of the human rights of immigrants.

\begin{figure}
\includegraphics[width=.8\textwidth]{./figures/4-6.png}
\caption{Collocates of sin papeles in esTenTen11, sorted by logDice in SketchEngine}
\end{figure}
...
\noindent (27) \newline
\noindent \textbf{[EN]} - You wanna buy these Chinamen? - Don't be ignorant. They're Thai or Cambodian. Entirely different kind of \textit{chinks}.\newline
\noindent \textbf{[ES]} -¿Vas a comprar esos chinos? -No seas ignorante. Serán tailandeses o camboyanos. Son unos \textit{amarillos} distintos.\newline
\noindent \textbf{[Back translation]} Should be Thai or Cambodian. They are different yellow people.\newline
